...
test('selects, moves and removes multiple deck words', async ({ page }) => {
  ...
  expect(selectionBox!.height).toBeLessThan(62)
  expect(navBox!.y - (selectionBox!.y + selectionBox!.height)).toBeGreaterThan(8)
  expect(navBox!.y - (selectionBox!.y + selectionBox!.height)).toBeLessThan(90)
...
test('shows Training without flip-card and quiz blocks', async ({ page }) => {
  await page.goto('/training')
  await expect(page.getByRole('button', { name: /Изучение/ })).toBeVisible()
...
test('uses application controls for profile settings', async ({ page }) => {
  ...
  await expect(page.getByRole('slider', { name: 'Дневная цель от 0 до 100 слов' })).toHaveCount(0)
  await page.getByRole('button', { name: /Дневная цель/ }).click()
  const dialog = page.getByRole('dialog', { name: 'Дневная цель' })
  const slider = dialog.getByRole('slider', { name: 'Дневная цель от 0 до 100 слов' })
  await expect(slider).toHaveValue('30')
  await slider.fill('50')
  await expect(slider).toHaveValue('50')
  await dialog.getByRole('button', { name: 'Сохранить' }).click()
  await expect(page.getByRole('button', { name: /Дневная цель/ })).toContainText('50 слов')
})
...
test('creates a personal phrase, finds it in My and adds it to a deck', async ({ page }) => {
  ...
  await expect(page.getByText('Моё')).toHaveCount(0)
  const personalCardHeight = await page.locator('.personal-word-card').first().evaluate((element) => element.getBoundingClientRect().height)
  expect(personalCardHeight).toBeLessThanOrEqual(191)
  await page.locator('.personal-word-card a').first().click()
  await expect(page.getByText('Личная запись')).toBeVisible()
  await expect(page.getByText('Фраза')).toBeVisible()
  await expect(page.getByRole('button', { name: 'Озвучивание пока недоступно' })).toHaveCount(0)
  ...
  const actionsButton = page.getByRole('button', { name: 'Действия с фразой' })
  const [backBox, actionsBox] = await Promise.all([backButton.boundingBox(), actionsButton.boundingBox()])
  expect(Math.abs(backBox!.y - actionsBox!.y)).toBeLessThan(3)
  await expect(page.getByRole('button', { name: 'Сохранить фразу' })).toBeVisible()
  await actionsButton.click()
  await expect(page.getByRole('dialog', { name: 'Действия с фразой' }).getByRole('button', { name: 'Изменить' })).toBeVisible()
  await page.keyboard.press('Escape')
  await page.goBack()
